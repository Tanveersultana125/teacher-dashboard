  ...
  const handleDownloadHistory = (h: any) => {
    try {
      ...
      const html = buildReport({
        ...
        // Identical phrasing across all 3 dashboards.
        subtitle: `Generated by ${h.generatedBy || "Principal"} · ${h.format || "PDF"} Format`,
        badge: h.className || h.grade || "",
        // Branch name first — that's the user-facing school identity in
        // the Edullent multi-branch model (a teacher's "school" is the
        // branch they teach at). Priority: doc.branchName (new publish) →
        // live teacherData.branchName (covers OLD reports without
        // branchName) → doc.schoolName → teacherData.schoolName.
        schoolName:
          h.branchName
